...
import { activateReferralForVerifiedUser, archiveModel, archiveProvider, banUserAccess, checkApiKeyPolicy, claimReferralRewards, clearLoginFailures, consumeAuthToken, createAnnouncement, createApiKey, createAuthToken, createCouponCode, createModel, createPendingReferral, createUser, deactivateCouponCode, getAutoRoutePolicy, updateAutoRoutePolicy, deleteAllUserSessions, getAnalytics, getCreditEconomy, getOverview, getRateLimitSettings, getReferralProgramMetrics, getReferralStats, getUserByEmail, getUserById, getUserForensics, getApiKeyPolicyUsage, listModelIdentities, updateApiKeyPolicy, updateModelIdentity, upsertModelIdentity, previewProviderModels, listAdminApiKeys, listAdminRequestLogs, listAdminSecurityEvents, listAdminUserLedger, listAdminUserSessions, listAdminUserUsage, listAnnouncements, listApiKeys, listCouponCodes, listModels, listProviderCredentials, listProviderHealth, listProviders, listRateLimitPolicies, listUsers, markEmailVerified, expireUserApiKeys, quarantineUserApiKeys, revokeAllUserApiKeys, rotateUserApiKeys, recordFailedLogin, recordLogin, recordSecurityEvent, redeemCouponCode, revokeApiKey, restoreModel, saveProvider, saveProviderCredential, saveRateLimitPolicy, saveRateLimits, seedDemoData, setAnnouncementActive, setApiKeyProviderAccess, setGlobalApiEnabled, setProviderCredentialActive, setRateLimitPolicyEnabled, setUserDisabled, syncProviderModels, takeRateLimit, testModelRoute, testModelSample, testProviderConnection, testProviderCredential, updateModel, updatePasswordAndRevokeSessions } from "./db";
...
export const appRouter = router({
  ...
  apiKeys: router({
    ...
    usage: protectedProcedure.input(z.object({ id: z.string().uuid() })).query(({ ctx, input }) => getApiKeyPolicyUsage(ctx.user.id, input.id)),
    create: protectedProcedure.input(z.object({ name: z.string().trim().min(2).max(80), expiresAt: z.string().datetime().nullable().optional(), creditLimit: z.string().regex(/^\d+(\.\d{1,3})?$/).nullable().optional(), requestLimitPerMinute: z.number().int().min(1).max(100000).nullable().optional(), tokenLimitPerMinute: z.number().int().min(100).max(100000000).nullable().optional() })).mutation(async ({ ctx, input }) => { const result = await createApiKey(ctx.user.id, input.name, { expiresAt: input.expiresAt ? new Date(input.expiresAt) : null, creditLimit: input.creditLimit ?? null, requestLimitPerMinute: input.requestLimitPerMinute ?? null, tokenLimitPerMinute: input.tokenLimitPerMinute ?? null }); await recordSecurityEvent({ eventType: "api_key_created", userId: ctx.user.id, metadata: { apiKeyId: result.id, name: input.name, expiresAt: input.expiresAt ?? null, hasCreditLimit: Boolean(input.creditLimit), hasRateLimits: Boolean(input.requestLimitPerMinute || input.tokenLimitPerMinute) } }); return result; }),
    updatePolicy: protectedProcedure.input(z.object({ id: z.string().uuid(), name: z.string().trim().min(2).max(80).optional(), expiresAt: z.string().datetime().nullable().optional(), creditLimit: z.string().regex(/^\d+(\.\d{1,3})?$/).nullable().optional(), requestLimitPerMinute: z.number().int().min(1).max(100000).nullable().optional(), tokenLimitPerMinute: z.number().int().min(100).max(100000000).nullable().optional() })).mutation(async ({ ctx, input }) => { const result = await updateApiKeyPolicy(ctx.user.id, input.id, { name: input.name, expiresAt: input.expiresAt === undefined ? undefined : input.expiresAt ? new Date(input.expiresAt) : null, creditLimit: input.creditLimit, requestLimitPerMinute: input.requestLimitPerMinute, tokenLimitPerMinute: input.tokenLimitPerMinute }); if (!result) throw new TRPCError({ code: "NOT_FOUND", message: "Active API key not found" }); await recordSecurityEvent({ eventType: "api_key_policy_updated", userId: ctx.user.id, metadata: { apiKeyId: input.id, hasExpiry: Boolean(input.expiresAt), hasCreditLimit: input.creditLimit !== undefined && input.creditLimit !== null, hasRateLimits: Boolean(input.requestLimitPerMinute || input.tokenLimitPerMinute) } }); return result; }),
    revoke: protectedProcedure.input(z.object({ id: z.string().uuid() })).mutation(async ({ ctx, input }) => { const success = await revokeApiKey(ctx.user.id, input.id); if (success) await recordSecurityEvent({ eventType: "api_key_revoked", userId: ctx.user.id, metadata: { apiKeyId: input.id } }); return { success }; }),
  }),
  coupons: router({
  ...
  admin: router({
    ...
    modelIdentities: adminProcedure.query(() => listModelIdentities()),
    upsertModelIdentity: adminProcedure.input(z.object({ slug: z.string().trim().min(2).max(120), displayName: z.string().trim().min(2).max(120), description: z.string().trim().max(500).optional(), isEnabled: z.boolean().optional() })).mutation(async ({ input, ctx }) => { const identity = await upsertModelIdentity(input); await recordSecurityEvent({ eventType: "founder_model_identity_upserted", userId: ctx.user.id, metadata: { modelIdentityId: identity.id, slug: identity.slug, isEnabled: identity.isEnabled } }); return identity; }),
    updateModelIdentity: adminProcedure.input(z.object({ id: z.number().int().positive(), displayName: z.string().trim().min(2).max(120).optional(), description: z.string().trim().max(500).optional(), isEnabled: z.boolean().optional() })).mutation(async ({ input, ctx }) => { const { id, ...values } = input; const identity = await updateModelIdentity(id, values); if (!identity) throw new TRPCError({ code: "NOT_FOUND", message: "Model identity not found" }); await recordSecurityEvent({ eventType: "founder_model_identity_updated", userId: ctx.user.id, metadata: { modelIdentityId: id, changed: Object.keys(values), isEnabled: values.isEnabled } }); return identity; }),
    createModel: adminProcedure.input(z.object({ slug: z.string().min(3).max(120), displayName: z.string().min(2).max(120), providerId: z.number().int().positive(), upstreamId: z.string().min(1).max(160), contextWindow: z.number().int().min(1).max(2_000_000), inputPrice: z.string().regex(/^\d+(\.\d{1,6})?$/), outputPrice: z.string().regex(/^\d+(\.\d{1,6})?$/), creditCostPer1kTokens: z.string().regex(/^\d+(\.\d{1,3})?$/).default("1"), isEnabled: z.boolean(), routingConfig: z.object({ protocol: z.enum(["openai", "anthropic", "gemini"]), priority: z.number().int().min(1).max(1000).default(100), fallbackProviderId: z.number().int().positive().optional(), capabilities: z.object({ streaming: z.boolean(), vision: z.boolean(), tools: z.boolean(), jsonMode: z.boolean(), reasoning: z.boolean() }), headers: z.record(z.string().regex(/^[a-zA-Z0-9-]+$/), z.string().max(200)).optional() }).optional() })).mutation(async ({ input, ctx }) => { const model = await createModel(input); await recordSecurityEvent({ eventType: "founder_model_created", userId: ctx.user.id, metadata: { modelId: model.id, providerId: input.providerId, isEnabled: input.isEnabled, routingConfig: input.routingConfig ? { protocol: input.routingConfig.protocol, priority: input.routingConfig.priority } : undefined } }); return model; }),
    updateModel: adminProcedure.input(z.object({ id: z.number().int().positive(), isEnabled: z.boolean().optional(), displayName: z.string().min(2).max(120).optional(), upstreamId: z.string().min(1).max(160).optional(), creditCostPer1kTokens: z.string().regex(/^\d+(\.\d{1,3})?$/).optional(), routingConfig: z.object({ protocol: z.enum(["openai", "anthropic", "gemini"]), priority: z.number().int().min(1).max(1000), fallbackProviderId: z.number().int().positive().optional(), capabilities: z.object({ streaming: z.boolean(), vision: z.boolean(), tools: z.boolean(), jsonMode: z.boolean(), reasoning: z.boolean() }), headers: z.record(z.string().regex(/^[a-zA-Z0-9-]+$/), z.string().max(200)).optional() }).optional() })).mutation(async ({ input, ctx }) => { const { id, ...values } = input; const model = await updateModel(id, values); await recordSecurityEvent({ eventType: "founder_model_updated", userId: ctx.user.id, metadata: { modelId: id, changed: Object.keys(values), isEnabled: values.isEnabled, routingConfigUpdated: Boolean(values.routingConfig) } }); return model; }),
    ...
    restoreModel: adminProcedure.input(z.object({ id: z.number().int().positive() })).mutation(async ({ input, ctx }) => { const model = await restoreModel(input.id); await recordSecurityEvent({ eventType: "founder_model_restored", userId: ctx.user.id, metadata: { modelId: input.id, slug: model.slug } }); return model; }),
    bulkModelLifecycle: adminProcedure.input(z.object({ ids: z.array(z.number().int().positive()).min(1).max(500), action: z.enum(["enable", "disable", "archive", "restore"]) })).mutation(async ({ input, ctx }) => { const results = []; for (const id of input.ids) results.push(input.action === "archive" ? await archiveModel(id) : input.action === "restore" ? await restoreModel(id) : await updateModel(id, { isEnabled: input.action === "enable" })); await recordSecurityEvent({ eventType: "founder_models_bulk_lifecycle", userId: ctx.user.id, metadata: { action: input.action, count: results.length } }); return { count: results.length, action: input.action }; }),
    ...
    autoRoutePolicy: adminProcedure.query(() => getAutoRoutePolicy()),
    updateAutoRoutePolicy: adminProcedure.input(z.object({ isEnabled: z.boolean().optional(), maxCostPer1k: z.string().regex(/^\d+(\.\d{1,3})?$/).optional(), latencyBudgetMs: z.number().int().min(1000).max(120000).optional(), minContextWindow: z.number().int().min(1).max(2_000_000).optional(), requireHealthy: z.boolean().optional(), fallbackOn5xx: z.boolean().optional(), fallbackOnTimeout: z.boolean().optional(), routingConfig: z.record(z.string(), z.unknown()).optional() })).mutation(async ({ input, ctx }) => { const result = await updateAutoRoutePolicy(input, ctx.user.id); await recordSecurityEvent({ eventType: "founder_auto_route_policy_updated", userId: ctx.user.id, metadata: { ...input, routingConfigKeys: Object.keys(input.routingConfig ?? {}) } }); return result; }),

...
import { activateReferralForVerifiedUser, archiveModel, archiveProvider, banUserAccess, claimReferralRewards, clearLoginFailures, consumeAuthToken, createAnnouncement, createApiKey, createAuthToken, createCouponCode, createModel, createPendingReferral, createUser, deactivateCouponCode, deleteAllUserSessions, getAnalytics, getCreditEconomy, getOverview, getRateLimitSettings, getReferralProgramMetrics, getReferralStats, getUserByEmail, getUserById, getUserForensics, listAdminApiKeys, listAdminRequestLogs, listAdminSecurityEvents, listAdminUserLedger, listAdminUserSessions, listAdminUserUsage, listAnnouncements, listApiKeys, listCouponCodes, listModels, listProviderCredentials, listProviderHealth, listProviders, listUsers, markEmailVerified, revokeAllUserApiKeys, recordFailedLogin, recordLogin, recordSecurityEvent, redeemCouponCode, revokeApiKey, saveProvider, saveProviderCredential, saveRateLimits, seedDemoData, setAnnouncementActive, setApiKeyProviderAccess, setGlobalApiEnabled, setProviderCredentialActive, setUserDisabled, syncProviderModels, takeRateLimit, testProviderConnection, testProviderCredential, updateModel, updatePasswordAndRevokeSessions } from "./db";
...
export const appRouter = router({
  ...
  admin: router({
    ...
    auditEvents: adminProcedure.input(z.object({ userId: z.number().int().positive().optional(), eventType: z.string().trim().max(120).optional(), from: z.string().date().optional(), to: z.string().date().optional(), limit: z.number().int().min(1).max(500).default(200) }).default({ limit: 200 })).query(({ input }) => listAdminSecurityEvents(input)),
